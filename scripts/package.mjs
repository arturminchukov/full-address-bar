...
const version = process.env.FAB_VERSION || base.version;
...
for (const target of ['chrome', 'firefox']) {
  ...
  const zipPath = join(releaseDir, `full-address-bar-${target}-${version}.zip`);
  rmSync(zipPath, { force: true });
  // Zip the folder contents (manifest.json at the archive root).
  execFileSync('zip', ['-qr', zipPath, '.'], { cwd: srcDir, stdio: 'inherit' });
  console.log(`packaged release/full-address-bar-${target}-${version}.zip`);
}
